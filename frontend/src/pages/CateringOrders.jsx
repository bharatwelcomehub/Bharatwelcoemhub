...
export default function CateringOrders() {
  ...
  const sendQuote = async (row) => {
    if (!row.email) {
      const m = window.prompt("Customer email not set on this order. Enter email to send to:");
      if (!m) return;
      row = { ...row, email: m };
    }
    const t = toast.loading(`Emailing quote to ${row.email}...`);
    try {
      const res = await fetch(`${API}/api/bookings/ext/catering/send-quote/${row.id}`, {
        method: "POST", headers: { "Content-Type": "application/json" },
                ...
                <TableRow key={r.id} data-testid={`catering-row-${r.id}`}>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      ...
                      <Button size="icon" variant="ghost" onClick={() => sendQuote(r)} title="Send quote to customer" data-testid={`catering-send-quote-${r.id}`}>
                        {r.quote_sent_at
